...
describe('aiService', () => {
  beforeEach(() => {
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, options) => {
      ...
      if (msg.includes('where am i')) text = 'You are near Gate 2. I can help you find your way.';
        ...
        text =
          body.context?.amenities?.length === 0
            ? 'undefined'
            : 'You can get a Burger at the East food stand.';
      else if (msg.includes('toilet') || msg.includes('restroom'))
        text =
          body.context?.amenities?.length === 0
            ? 'undefined'
            : 'The nearest one is Restroom 1 in the South concourse.';
        ...
        if (body.mode === 'volunteer')
          text = 'Volunteer mode active. Use this for incident reporting.';
        if (body.mode === 'organizer') text = 'Command mode active.';
      } else if (msg.includes('report incident'))
        text = 'Please provide the exact location of the incident.';
      ...
      } else if (msg.includes('successful api response for test'))
        text = 'Successful API response for test';
  ...
  it('should fall back to mock logic when API fails or no key is present', async () => {
    const response = await generateAIResponse('lost', {
      role: 'fan' as any,
      stadiumState: mockData,
    });
  ...
  describe('fan responses', () => {
    it('handles lost queries', async () => {
      const res = await generateAIResponse('where am i', {
        role: 'fan' as any,
        stadiumState: mockData,
      });
    ...
    it('handles gate queries', async () => {
      const res = await generateAIResponse('find gate', {
        role: 'fan' as any,
        stadiumState: mockData,
      });
    ...
    it('handles food queries', async () => {
      const res = await generateAIResponse('hungry', {
        role: 'fan' as any,
        stadiumState: mockData,
      });
    ...
    it('handles restroom queries', async () => {
      const res = await generateAIResponse('toilet', {
        role: 'fan' as any,
        stadiumState: mockData,
      });
    ...
    it('handles seat queries', async () => {
      const res = await generateAIResponse('my seat', {
        role: 'fan' as any,
        stadiumState: mockData,
      });
    ...
    it('handles wheelchair queries', async () => {
      const res = await generateAIResponse('accessible', {
        role: 'fan' as any,
        stadiumState: mockData,
      });
  ...
  describe('volunteer responses', () => {
    it('handles incident queries', async () => {
      const res = await generateAIResponse('report incident', {
    ...
    it('handles translate queries', async () => {
      const res = await generateAIResponse('translate', {
        role: 'volunteer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles map queries', async () => {
      const res = await generateAIResponse('map', {
        role: 'volunteer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles task queries', async () => {
      const res = await generateAIResponse('task', {
        role: 'volunteer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles crowd queries', async () => {
      const res = await generateAIResponse('density', {
        role: 'volunteer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles default volunteer query', async () => {
      const res = await generateAIResponse('hello', {
        role: 'volunteer' as any,
        stadiumState: mockData,
      });
  ...
  describe('organizer responses', () => {
    it('handles gate 4 queries', async () => {
      const res = await generateAIResponse('gate 4', {
    ...
    it('handles broadcast queries', async () => {
      const res = await generateAIResponse('broadcast', {
        role: 'organizer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles incident queries', async () => {
      const res = await generateAIResponse('incident', {
        role: 'organizer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles wait queries', async () => {
      const res = await generateAIResponse('wait', {
        role: 'organizer' as any,
        stadiumState: mockData,
      });
    ...
    it('handles staff queries', async () => {
      const res = await generateAIResponse('staff', {
        role: 'organizer' as any,
        stadiumState: mockData,
      });
  ...
  it('handles completely unrecognised queries', async () => {
    const res = await generateAIResponse('ajsdklfjsdf', {
      role: 'organizer' as any,
      stadiumState: mockData,
    });
    expect(res).toContain('Command mode active');
    const res2 = await generateAIResponse('ajsdklfjsdf', {
      role: '' as any,
      stadiumState: mockData,
    });
  ...
  it('handles empty amenities gracefully', async () => {
    ...
    const resFood = await generateAIResponse('food', {
      role: 'fan' as any,
      stadiumState: emptyData,
    });
    expect(resFood).toContain('undefined'); // Because it returns undefined name/location if not found

    const resRestroom = await generateAIResponse('restroom', {
      role: 'fan' as any,
      stadiumState: emptyData,
    });
  ...
  it('returns API response when successful', async () => {
    shouldFail = false;
    const res = await generateAIResponse('successful api response for test', {
      role: 'fan' as import('../types').UserRole,
      stadiumState: mockData,
    });
