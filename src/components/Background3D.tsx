...
const FloatingParticles = () => {
  ...
  const { positions, colors, speeds } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const mixedColor = rand > 0.8 ? colorGreen : rand > 0.4 ? colorTeal : colorDark;
...
const AbstractStadiumFloor = () => {
  ...
  return (
    <group position={[0, -18, -10]}>
      ...
      <gridHelper ref={gridRef} args={[150, 40, '#2FBF9F', '#0B0F14']} position={[0, 0, 0]} />
...
export const Background3D: React.FC = () => {
  return (
    <div className="fixed inset-0 z-[-1] pointer-events-none bg-brand-dark" aria-hidden="true">
      <Canvas camera={{ position: [0, 5, 20], fov: 60 }} style={{ pointerEvents: 'none' }}>
