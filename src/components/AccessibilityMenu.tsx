...
const AccessibilityMenu: React.FC = () => {
  ...
  const { language, setLanguage, highContrast, setHighContrast, fontSize, setFontSize } =
    useAppStore();
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                <span className="flex items-center gap-2">
                  <Contrast size={16} /> {t.highContrast}
                </span>
                <div
                  className={`w-8 h-4 rounded-full transition-colors ${highContrast ? 'bg-brand-teal' : 'bg-slate-600'} relative`}
                >
                  <div
                    className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${highContrast ? 'translate-x-4' : ''}`}
                  />
                </div>
              </button>
              ...
              >
                <span className="flex items-center gap-2">
                  <Type size={16} /> {t.largeText}
                </span>
                <div
                  className={`w-8 h-4 rounded-full transition-colors ${fontSize === 'large' ? 'bg-brand-teal' : 'bg-slate-600'} relative`}
                >
                  <div
                    className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${fontSize === 'large' ? 'translate-x-4' : ''}`}
                  />
                </div>
              </button>
