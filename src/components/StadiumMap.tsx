...
const LOCATIONS: Record<string, [number, number]> = {
  ...
  'VIP Lounge': [25.2775, 55.297],
  'Fan Seat (Block B, Row 4)': [25.276, 55.2955],
};
...
const StadiumMap: React.FC = () => {
  ...
  const routingType =
    (fromLocation.includes('Seat') ||
      fromLocation.includes('Gate') ||
      fromLocation.includes('VIP')) &&
  ...
  return (
    <div className="glass-panel p-6 h-full flex flex-col">
      ...
      >
        <div
          className="flex-1 w-full relative min-h-[300px] overflow-hidden rounded-xl border border-white/5"
          style={{ zIndex: 0 }}
        >
          ...
          {viewMode === 'google' && isRoutingMode && (
            <div className="absolute top-12 left-1/2 -translate-x-1/2 z-[400] bg-brand-dark/95 backdrop-blur-md p-3 rounded-xl border border-white/10 shadow-2xl w-[90%] max-w-sm animate-in fade-in slide-in-from-top-4 duration-300">
              <h3 className="text-white text-sm font-medium mb-2 flex items-center gap-1.5">
                <MapPin size={14} className="text-brand-teal" /> Navigation
              </h3>
              <div className="flex flex-col gap-2">
                <div>
                  <label className="text-[10px] text-white/50 mb-0.5 block font-medium uppercase tracking-wider">
                    From
                  </label>
                  <select
                    className="w-full bg-black/60 border border-white/10 rounded-lg p-1.5 text-xs text-white focus:border-brand-teal focus:ring-1 focus:ring-brand-teal outline-none transition-all"
                    value={fromLocation}
          ...
          {viewMode === '3d' ? (
            ...
            >
              ...
              {isRoutingMode && (
                <MapRouter
                  fromPoint={LOCATIONS[fromLocation]}
                  toPoint={LOCATIONS[toLocation]}
